...
public:
	...
	void updateCurrentPointsToMap(const pcl::PointCloud<PointType>::Ptr &pc_in, const Eigen::Isometry3d &pose_current);
	pcl::PointCloud<PointType>::Ptr getMap(void);
...
private:
	...
	std::vector<std::vector<std::vector<pcl::PointCloud<PointType>::Ptr>>> map;
	...
	void noiseFilter(const pcl::PointCloud<PointType>::Ptr &pc_in, pcl::PointCloud<PointType>::Ptr &pc_out);
};
