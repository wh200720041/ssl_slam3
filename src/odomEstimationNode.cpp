    ...
    while (true)
    {
        ...
        if (last_synchronization_time == 0.0)
        {
            ...
            while (!imu_buf.empty() && imu_buf.front()->header.stamp.toSec() < lidar_time)
                imu_buf.pop();
            pcl::PointCloud<PointType>::Ptr edge_points_in(new pcl::PointCloud<PointType>());
            pcl::PointCloud<PointType>::Ptr surf_points_in(new pcl::PointCloud<PointType>());
...
void odom_estimation()
{
    while (true)
    {
        ...
        else
        {
            ...
            double t = tk.toc();
            timer.toc(10);
            std::cout << "process a scan takes : " << t << "ms" << std::endl;
            std::cout << "-----------------------------------" << std::endl;
        }
        ...
        // std::cout << "odom: " << t_current.transpose() << std::endl;
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh("~");
    ROS_INFO("\033[1;32m----> Odom Estimation Started.\033[0m");
    std::string file_path;
    nh.getParam("/file_path", file_path);
    odom_estimator.init(file_path);

    ros::Subscriber sub_imu = nh.subscribe<sensor_msgs::Imu>("/os_cloud_node/imu", 100, imuCallback);
