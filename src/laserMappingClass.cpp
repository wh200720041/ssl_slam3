...
void LaserMappingClass::init(std::string &file_path)
{
	...
	{
		std::vector<std::vector<pcl::PointCloud<PointType>::Ptr>> map_height_temp;
		for (int j = 0; j < lidar_param.getMapCellHeightRange() * 2 + 1; j++)
		{
			std::vector<pcl::PointCloud<PointType>::Ptr> map_depth_temp;
			for (int k = 0; k < lidar_param.getMapCellDepthRange() * 2 + 1; k++)
			{
				pcl::PointCloud<PointType>::Ptr point_cloud_temp(new pcl::PointCloud<PointType>);
...
void LaserMappingClass::addWidthCellNegative(void)
{
	...
	for (int j = 0; j < map_height; j++)
	{
		...
		{
			pcl::PointCloud<PointType>::Ptr point_cloud_temp;
			map_depth_temp.push_back(point_cloud_temp);
		}
		map_height_temp.push_back(map_depth_temp);
	}
...
void LaserMappingClass::noiseFilter(const pcl::PointCloud<PointType>::Ptr &pc_in, pcl::PointCloud<PointType>::Ptr &pc_out)
{
	...
	pcl::PointCloud<PointType>::Ptr pc_without_ceiling(new pcl::PointCloud<PointType>());
	...
	pcl::PointCloud<PointType>::Ptr down_sized_pc(new pcl::PointCloud<PointType>());
...
void LaserMappingClass::updateCurrentPointsToMap(const pcl::PointCloud<PointType>::Ptr &pc_in, const Eigen::Isometry3d &pose_current)
{
	...
	pcl::PointCloud<PointType>::Ptr transformed_pc(new pcl::PointCloud<PointType>());
	...
	{
		PointType point_temp = transformed_pc->points[i];
...
pcl::PointCloud<PointType>::Ptr LaserMappingClass::getMap(void)
{
	pcl::PointCloud<PointType>::Ptr laserCloudMap = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>());
	for (int i = 0; i < map_width; i++)
	{
		for (int j = 0; j < map_height; j++)
		{
